...
function connect() {
    ...
    stompClient.connect({'Auth-Token': token}, function (frame) {
        ...
        listenOrderSubscription = stompClient.subscribe('/topic/hailingService/trip/publishTrip', function (trip) {
...
function sendLocation() {
    ...
    stompClient.send("/api/hailingService/car/uploadCarLocation", {}, JSON.stringify(carLocation));

    // 直接将消息发送给订阅/topic/hailingService/car/uploadCarLocation的前端，不需要通过后端处理
    // stompClient.send("/topic/hailingService/car/uploadCarLocation",{}, JSON.stringify(location));
}
